...
#    N = int(f.readline().split()[0])
    R = int(f.readline().split()[0])
    num_points = int(f.readline().split()[0])
#    smof = []
#    for line in f:
#        if ',' in line:
#            line = line.rstrip('\n')
#            smof = list(map(int, line.split(',')))

    return A, B, smof, N, R, num_points
...
#A, B, smof, N, R, num_points = parseinput('sys_info')
...
#    print('#', 'Steps', 'nv1', 'nv2', 'nv3', 'nv4', 'nv5', 'nv6', 'nv7', 'nv8', 'nv9', 'nv10', file=out)
...
#for i in range(100):
# pltdata(1, 2)
#    os.rename('2d-walk.dat', 'temp'+str(i)+'.dat')
#
